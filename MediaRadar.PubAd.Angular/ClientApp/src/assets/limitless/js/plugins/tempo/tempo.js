...
var Tempo = (function (tempo) {
    ...
    function Templates(params, nestedItem) {
        ...
        if (typeof params !== 'undefined') {
            for (var prop in params) {
                ...
                } else if (prop === 'enableEscape') {
                    this.escape = params[prop];
    ...
    Renderer.prototype = {
        ...
        filters: {
            ...
            'last4chars': function (value, args) {
                if (!value || value.toString() == "")
                    return value;
                else {
                    var strValue = value.toString();
                    var startIndex = Math.min(strValue.length, 4);
                    return "*****" + strValue.substr(strValue.length - startIndex, startIndex);
                }
            },
            'dateDbSet': function (value, args) {
                var value_df = new Date(parseInt(value.substr(6)));
                return $.datepicker.formatDate('mm/dd/yy', value_df);
            },
            'dateMY': function (value, args) {
                var value_df = new Date(parseInt(value.substr(6)));
                return $.datepicker.formatDate('MM yy', value_df);
            },
            'dateMMDDYYYY': function (value, args) {
                var value_df = new Date(parseInt(value.substr(6)));
                return $.datepicker.formatDate('M dd, yy', value_df);
            },
            'datetime': function (value, args) {
                var value_df = new Date(parseInt(value.substr(6)));
                return $.datepicker.formatDate('M dd, yy', value_df);
            },
            'currency': function (value, args) {
                ...
                return (neg ? "-$" : '$') + parseFloat(inputNumber, 10).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,").toString();
